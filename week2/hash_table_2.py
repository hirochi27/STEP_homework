...
#別のifの中でやっちゃってるから出した方がいい。あと、途中Returnで返しちゃってる
#連結リストの最初に突っ込む
#put in putを辞める　→　4行くらいで
        ...
        if self.item_count  > self.bucket_size * 0.7:
            ...
            if self.bucket_size % 2 == 0: #
                #バケット数が偶数になった場合、奇数にする
                self.bucket_size += 1        
            ...
            for item in old_buckets:
                while item != None:
                    ...
                    hash_number = calculate_hash(put_item.key)
                    hash = hash_number % self.bucket_size

                    #if self.buckets[hash] != None:
                    put_item.next = self.buckets[hash]
                    self.buckets[hash] = put_item
                    #else:
                        #self.buckets[hash] = put_item
                        #put_item.next = None
